...
func init() {
	...
	globalMemoryStatusEx = libkernel32.NewProc("GlobalMemoryStatusEx")
...
func GetThreadLocale() LCID {
	ret, _, _ := syscall.Syscall(getThreadLocale.Addr(), 0, 0, 0, 0)
	return LCID(ret)
}
...
func GetThreadUILanguage() LANGID {
	...
	ret, _, _ := syscall.Syscall(getThreadUILanguage.Addr(), 0, 0, 0, 0)
	return LANGID(ret)
}

func GetVersion() uint32 {
	ret, _, _ := syscall.Syscall(getVersion.Addr(), 0, 0, 0, 0)
	return uint32(ret)
}
...
func GlobalMemoryStatusEx(memInfo uintptr) uint32 {
	ret, _, _ := syscall.Syscall(globalMemoryStatusEx.Addr(), 1, uintptr(memInfo), 0, 0)
	return uint32(ret)
}
